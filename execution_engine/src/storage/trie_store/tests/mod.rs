...
fn create_data() -> Vec<TestData<Bytes, Bytes>> {
    ...
    let leaf_1_hash = Blake2bHash::new(&bytesrepr::serialize(&leaf_1).unwrap());
    let leaf_2_hash = Blake2bHash::new(&bytesrepr::serialize(&leaf_2).unwrap());
    let leaf_3_hash = Blake2bHash::new(&bytesrepr::serialize(&leaf_3).unwrap());
    ...
    let node_2_hash = Blake2bHash::new(&bytesrepr::serialize(&node_2).unwrap());
    ...
    let ext_node_hash = Blake2bHash::new(&bytesrepr::serialize(&ext_node).unwrap());
    ...
    let node_1_hash = Blake2bHash::new(&bytesrepr::serialize(&node_1).unwrap());
